...
if (!is_null($objMdUtlControleDsmpDTO)) {
    ...
    if ($numRegistros > 0) {
        ...
        for ($i = 0; $i < $numRegistros; $i++) {
            ...
            $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_utl_ajuste_prazo_consultar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_ajuste_prazo=' . $numIdAjustePrazo . '&id_controle_desempenho=' . $numIdControleDsmp . '&is_gerir=0') . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/consultar.svg" title="Consultar Solicitação de Ajuste de Prazo" alt="" class="infraImg" /></a>&nbsp;';
            $strResultado .= '<a href="' . SessaoSEI::getInstance()->assinarLink('controlador.php?acao=md_utl_ajuste_prazo_alterar&acao_origem=' . $_GET['acao'] . '&acao_retorno=' . $_GET['acao'] . '&id_ajuste_prazo=' . $numIdAjustePrazo . '&id_controle_desempenho=' . $numIdControleDsmp . '&is_gerir=1') . '" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="' . PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/alterar.svg" title="Alterar Solicitação de Ajuste de Prazo" alt="" class="infraImg" /></a>&nbsp;';
            $strResultado .= '<a id="aprovarSolicitacao" onclick="confirmarAcao(\'' . MdUtlAjustePrazoRN::$APROVADA . '\',\'' . $strProcesso . '\', \'' . $linkAprovar . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="'. PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/transportar.svg" title="Aprovar Solicitação de Ajuste de Prazo" alt="" class="infraImg" /></a>&nbsp;';
            $strResultado .= '<a id="reprovarSolicitacao" onclick="confirmarAcao(\'' . MdUtlAjustePrazoRN::$REPROVADA . '\',\'' . $strProcesso . '\', \'' . $linkReprovar . '\');" tabindex="' . PaginaSEI::getInstance()->getProxTabTabela() . '"><img src="'. PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/remover.svg" title="Reprovar Solicitação de Ajuste de Prazo" alt="" class="infraImg" /></a>&nbsp;';
            $strResultado .= '</td>';
