...
if ($isBuscaDados) {
    ...
    if ($idAtendimento == MdUtlControleDsmpRN::$RASCUNHO_ANALISE || $idAtendimento == MdUtlControleDsmpRN::$RASCUNHO_CORRECAO_ANALISE) {
        $rascunho = 1;
    }
}
...
$membroResponsavelTriagem = MdUtlControleDsmpINT::getNomeUsuarioRespTriagAnaliseAval(
    $idObj,
    $objControleDsmpDTO->getStrStaAtendimentoDsmp(),
    MdUtlControleDsmpRN::$STR_TIPO_ACAO_TRIAGEM
);

$nmUsuarioTriagem = $membroResponsavelTriagem->getStrNome();
$idUsuarioResp    = $membroResponsavelTriagem->getNumIdUsuario();
...
if ($isRetriagem) {
    $linkCancelar = "controlador.php?acao=";
    $linkCancelar .= $isRtgAnlCorrecao
        ? "md_utl_analise_alterar&acao_origem={$_GET['acao_origem']}&isRtgAnlCorrecao=1"
        : "md_utl_analise_cadastrar&acao_origem={$_GET['acao_origem']}";
    $linkCancelar .= "&id_procedimento=$idProcedimento&id_fila=$idFilaAtiva&id_retriagem=1";
} else {
    $linkCancelar = "controlador.php?acao={$_GET['acao_origem']}";
    $linkCancelar .= $isMeusProcessos == false ? "&id_procedimento=$idProcedimento" : "";
}
$strLinkCancelar = SessaoSEI::getInstance()->assinarLink($linkCancelar);
...
// retorna os Tipos de Controle que o usuario faz parte para uso das labels que retornam o tempo de execucao, distribuidas, etc
            ...
            if (!is_null($idProcedimento)) {
                try {
                    ...
                    if ($idRetriagem == 1) {
                        ...
                        } else if ($isPossuiAnalise == 'N') {
                            $url = 'controlador.php?acao=' . $paginaRetorno . '&id_procedimento=' . $idProcedimento . '&is_processo_concluido' . $isProcessoConcluido[0];
    ...
    <form onsubmit="return onSubmitForm();" id="frmUtlTriagemCadastro" method="post"
        ...
        <?php if ($isPgPadrao != 0) { ?>
            <div class="row mb-3">
                <div class="col-12">
                    ...
                    <a href="<?= $linkProcedimento ?>" target="_blank"
                       alt="Acessar o processo: <?= $strNumeroProcesso ?>"
                       title="Acessar o processo: <?= $strNumeroProcesso ?>" class="ancoraPadraoAzul">
                        <?= $strNumeroProcesso ?>
                    </a>
                </div>
            </div>
        ...
        <div class="row">
            <div class="col-sm-6 col-md-5 col-lg-4 mb-2">
                ...
                <input type="text" id="txtTipoControle" name="txtTipoControle" class="infraText form-control"
                       value="<?= $objControleDsmpDTO->getStrNomeTpControle() ?>" disabled/>
            </div>

            <div class="col-sm-6 col-md-5 col-lg-4 mb-2">
                <label id="lblFila" accesskey="" class="infraLabelOpcional">Fila:</label>
                <input type="text" id="txtNomeFila" name="txtNomeFila" class="infraText form-control"
                       value="<?= $objControleDsmpDTO->getStrNomeFila() ?>" disabled/>
            </div>

            <div class="col-sm-12 col-md-12 col-lg-4 mb-2">
                <label class="infraLabelOpcional">Membro Responsável pela Triagem:</label>
                <input type="text" value="<?= $nmUsuarioTriagem ?>" readonly class="infraText form-control">
            </div>
        </div>

        <?php require_once 'md_utl_triag_analise_rev_calculo_tempo.php' ?>

        <div id="divGrupoAtividade" class="mb-3" <?php if (!$existeGrupoCadastrado || $isConsultar) {
            echo 'style="display:none"';
        } ?>>
            <div class="row mb-1">
                <div class="col-xs-4 col-sm-6 col-md-6 col-lg-6">
                    <label id="lblGrupoAtividade" for="selGrupoAtividade" accesskey="" class="infraLabelOpcional">Grupo
                        de Atividade:</label>
                    <img id="btnGrupoAtividade" align="top"
            ...
            <div class="row">
                <div class="col-sm-9 col-md-9 col-lg-9">
                    <div class="input-group">
                        <select id="selGrupoAtividade" name="selGrupoAtividade" multiple="multiple"
                                class="infraSelect form-control">
                            <?= $strItensSelGrupoAtividade ?>
                        </select>
                        <div id="divOpcoesAtividade">
                            <div id="divOpcoesGrupoAtividade" class="ml-1">
                                <img id="imgLupaGrupoAtividade" onclick="abrirGrupoAtividade();"
                                     src="<?= PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/pesquisar.svg' ?>"
                                     alt="Selecionar Grupo de Atividade" title="Selecionar Grupo de Atividade"
                                     class="infraImg"/>
                                <br>
                                <img id="imgExcluirGrupoAtividade" onclick="removerGrupoAtividade()"
                                     src="<?= PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/remover.svg' ?>"
                                     alt="Remover Grupo de Atividade Selecionada"
                                     title="Remover Grupo de Atividade Selecionada" class="infraImg"/>
                            </div>
                        </div>
        ...
        <div id="divAtividade" class="mb-3" <?= $isConsultar ? 'style="display:none"' : '' ?>>
            <div class="row">
                <div class="col-xs-4 col-sm-10 col-md-10 col-lg-6">
                    <label id="lblAtividade" for="selAtividade" accesskey=""
                           class="infraLabelObrigatorio">Atividades:</label>
                    <img id="btnAtividade" align="top"
                         src="<?= PaginaSEI::getInstance()->getDiretorioSvgGlobal() ?>/ajuda.svg" class="infraImg"
            ...
            <div class="row">
                <div class="col-sm-12 col-md-12 col-lg-9">
                    <div class="input-group">
                        <select id="selAtividade" name="selAtividade" multiple="multiple"
                                class="infraSelect form-control">
                            <?= $strItensSelAtividade ?>
                        </select>
                        <div id="divOpcoesAtividade" class="ml-1">
                            <img id="imgLupaAtividade" onclick="selecionarAtividade();"
                                 src="<?= PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/pesquisar.svg' ?>"
                            ...
                            <img id="imgExcluirAtividade" onclick="objLupaAtividade.remover();"
                                 src="<?= PaginaSEI::getInstance()->getDiretorioSvgGlobal() . '/remover.svg' ?>"
                                 alt="Remover Atividade Selecionada" title="Remover Atividade Selecionada"
                                 class="infraImg"/>
                        </div>
                        <input type="hidden" id="hdnAtividade" name="hdnAtividade" value=""/>
                        <input type="hidden" id="hdnIdAtividade" name="hdnIdAtividade" value=""/>
                        <input type="hidden" id="hdnContadorTableAtv" name="hdnContadorTableAtv"
                               value="<?= $hdnContadorPagina ?>"/>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="row mb-3" <?= $isConsultar ? 'style="display:none"' : '' ?>>
            <div class="col-xs-2 col-sm-12 col-md-12 col-lg-9 text-right">
                ...
                <button <?= $disabled ?> type="button" class="infraButton" id="btnAdicionar" accesskey="a"
                                         onclick="adicionarRegistroTabelaAtividade();">
                    <span class="infraTeclaAtalho">A</span>dicionar
                </button>
            </div>
        </div>
        <div class="row" id="membroResponsavelAvaliacaoRow" style="<?= $displayMembroResponsavelAvaliacao ?>">
            <div class="col-xl-4 col-lg-4 col-md-6 col-sm-6 col-xs-10">
                <div id="divMembroResponsavelAvaliacao" class="form-group mb-3 mt-2">
                    <label id="lblUsuarioResponsavelAvaliacao" for="selUsuarioResponsavelAvaliacao" accesskey="">
                        Membro Responsável pela Avaliação:
                    </label>
                    <select id="selUsuarioResponsavelAvaliacao" name="selUsuarioResponsavelAvaliacao"
        ...
        <div id="divPrincipalEncaminhamento" class="row mb-3" style="<?= $displayEncaminhamento ?>">
            <div id="divEncaminhamentoTriagem" class="col-xs-12 col-sm-6 col-md-6 col-lg-6 col-xl-6">
                <label for="selEncaminhamentoTriagem" id="lblEncaminhamentoTriagem" class="infraLabelObrigatorio">Encaminhamento
                    da Triagem:</label>
                ...
                <select onchange="controlarExibicaoFila(this);" id="selEncaminhamentoTriagem"
                        class="infraSelect form-control" name="selEncaminhamentoTriagem">
                    <?= $selEncaminhamentoTriagem; ?>
                </select>
            </div>

            <div id="divFila" style="<?= $displayFila ?>"
                 class="col-xs-12 col-sm-6 col-md-6 col-lg-6 mb-3 pt-2 col-xl-6">
                ...
                <select id="selFila" name="selFila" class="infraSelect form-control"
                        onchange="distribuicaoAutoParaMim(this , 1 , <?= SessaoSEI::getInstance()->getNumIdUsuario() ?>)">
                    <?= $selFila; ?>
                </select>
            </div>
        ...
        <div class="row mb-4">
            <div class="col-12">
                ...
                <input type="hidden" name="hdnTbAtividade" id="hdnTbAtividade" utlCampoObrigatorio="a"
                       value="<?= $strGridTriagem; ?>"/>
            </div>
        </div>
            ...
            <div class="col-xs-1 col-sm-4 col-md-3 col-lg-3">
                ...
                <div class="input-group">
                    ...
                    <img src="<?= PaginaSEI::getInstance()->getDiretorioSvgGlobal() ?>/calendario.svg"
                         id="imgPrazoResposta" title="Selecionar Prazo para Resposta"
        ...
        <div class="row">
            <div class="col-12">
                ...
                <textarea id="txaInformacaoComplementar" name="txaInformacaoComplementar" rows="4"
        ...
        <input type="hidden" name="hdnStaPermiteAssociarFila" id="hdnStaPermiteAssociarFila"
               value="<?= MdUtlControleDsmpRN::$ENC_ASSOCIAR_EM_FILA ?>"/>
        ...
        <input type="hidden" id="hdnAtividadesAnalisadas"
               value="<?= !empty($arrAtvAnalisadas) ? implode(',', $arrAtvAnalisadas) : null ?>">
